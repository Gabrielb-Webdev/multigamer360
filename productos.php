                    ...
                    <?php if (!empty($products)): ?>
                        <?php foreach ($products as $product): ?>
                            ...
                            <div class="product-card" data-product-id="<?php echo $product['id']; ?>">
                                ...
                                <?php 
                                // DEBUG: Siempre mostrar qué trae la base de datos
                                echo "<!-- DEBUG Producto ID: " . $product['id'] . " | Name: " . htmlspecialchars($product['name'] ?? 'NO NAME') . " | main_image: " . ($product['main_image'] ?? 'NULL') . " -->\n";
                                
                                // Usar main_image de la tabla products
                                if (!empty($product['main_image'])) {
                                    $image_filename = $product['main_image'];
                                    echo "<!-- Usando main_image: " . htmlspecialchars($image_filename) . " -->\n";
                                } else {
                                    // Imagen por defecto
                                    $image_filename = 'product1.jpg';
                                    echo "<!-- Usando fallback: " . htmlspecialchars($image_filename) . " -->\n";
                                }
                                
                                // Construir la ruta de la imagen
                                $product_image = 'uploads/products/' . $image_filename;
                                echo "<!-- Ruta final: " . htmlspecialchars($product_image) . " -->\n";
                                ?>
                                ...
                                <!-- Franja de consola en la parte inferior -->
                                <div class="console-badge-bottom">
                                    <?php 
                                    // Obtener el nombre de la consola
                                    $console_display = !empty($product['console_name']) ? $product['console_name'] : 
                                                      (!empty($product['category_name']) ? $product['category_name'] : 'Videojuego');
                                    echo htmlspecialchars($console_display);
                                    ?>
                                </div>
...
.product-overlay {
    position: absolute;
    bottom: 34px; /* Dejar espacio para la franja de consola */
    left: 0;
    right: 0;
...
.product-card:hover .product-overlay {
    ...
    bottom: 0; /* Ocupar todo el espacio cuando la franja se oculta */
}
...
/* Franja de consola en la parte inferior de la card */
.console-badge-bottom {
    position: absolute;
    bottom: 0;
    left: 0;
    right: 0;
...
.product-card:hover .console-badge-bottom {
